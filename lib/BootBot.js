...
class BootBot extends EventEmitter {
   ...
   * @param {String} [options.ssl=false]
   * @param {String} [options.certFile=./cert/cert.pem]
   * @param {String} [options.keyFile=./cert/key.pem]
   */
  constructor(options) {
    super();
  ...
  start(port) {
    ...
    if (this.ssl) {
      this.server = https.createServer({
        ...
        console.log('BootBot running on port', 443);
      });
    } else {
